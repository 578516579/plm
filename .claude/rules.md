...
**Fast-track 例外条款**（proposal 0030 落地范本,2026-05-28）:满足下列**全部 5 条**的 hook / 工具链类小提案（号段 0200-0299）允许「同一会话内 proposal draft + 实装 + dogfood + 自检 + 提请 review」一次性完成,无需 draft → proposed → accepted 多轮流转:

1. **改动面**:≤ 1 个 hook 脚本 ± ≤ 1 配置文件(`.claude/hooks/*.sh` / `.githooks/*` / `.claude/settings.json` 范围)
2. **证据强度**:复现事故 ≥ 2 次 commit hash 可链(避免凭直觉 over-engineering)
3. **自检**:proposal §10 列出 ≥ 5 个测试场景 + 实际 stderr/exit 全粘贴
4. **Dogfood**:本会话同期 commit 自身用新机制验证(展示 protocol-perfect 流程)
5. **状态机**:proposal 状态先标 `draft → implementing`,等用户 review 转 `merged`(用户 1 行签字即可,不要求另起 PR)

**Fast-track 范本**:[proposal 0030 race add-all hard-block](../99-跨阶段/proposals/0030-race-add-all-hard-block.md) — session-guard.sh nudge → 硬拦,事故 `3ae00fd` + `656a6a4` 双 hash,5 自检测试全绿,2 commit dogfood。**不适用范围**:架构 / PRD / Gate Checklist 模板 / 模块工作流改动 — 这些必须走完整 proposal 流程。
...
### L.5 并行 session 共享 working tree 防 race（MUST）

**适用场景**:多 session（无论本地多终端 / Claude 跨会话 / 串行重启续作）在同一物理 working tree 工作时，git 暂存区是**进程级**资源 — `git add . / -A / -u / commit -a*` 会卷进所有脏文件，**包括别 session 的 WIP**。

**硬规则**:
1. **永远显式列文件路径**:`git add <path1> <path2> ...`，不准用通配 `git add .` / `-A` / `-u` / `--all` / `commit -a*`
2. **合法 bulk 场景**（epic 多模块 commit / bulk-refactor / 模板化改造）:
   - 单次 Bash 调用内 `export CLAUDE_BULK_OK="<≥10 字 reason>" && <bulk 命令>`
   - reason 推荐格式:`<proposal#> <scope>: <N 文件> <原因摘要>`
   - 例:`export CLAUDE_BULK_OK="0028 P0-3A+B: 30 文件 testreport+dora 后端聚合" && git add ...`
3. **紧急绕过**:`export CLAUDE_BYPASS_SESSION_GUARD=1 && <命令>` — 计入 signals 月度 bypass，> 1/月触发审视

**机制实现**:[.claude/hooks/session-guard.sh](hooks/session-guard.sh) PreToolUse hook，bulk add + working tree 有 ≥1 个脏文件 → `exit 2` 硬拦，Claude 工具调用失败。详 [proposal 0030](../99-跨阶段/proposals/0030-race-add-all-hard-block.md) + [project-quirks Q-COLLAB-01](../memory/project-quirks.md)。

**事故案例**:2026-05-28 epic 0028 单日 2 次:`3ae00fd` 偷 P0-1 22 文件 + `656a6a4` 偷 P0-2C 11 文件，均因 commit msg subject 与实际改动失配在事后 docs commit 显形。proposal 0008 留 nudge 已证失效 → 升级硬拦。
