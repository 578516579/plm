...
| [0028](0028-product-mainline-uplift-epic.md) | 产品主线贯通迭代 epic — P0-1 跨模块外键 + P0-2 详情页跨模块跳转 + P0-3 TestReport/DORA 真聚合 + P0-4 AiButton 紫渐变 + P0-5 Dashboard 错态显形 | **🎉 epic 100% merged → tracking**(User-requested,solo-review;2026-05-28 单日完工)| 2026-05-28 | 2026-05-28 PM 验收会话:5 主线 4 断 + AI 按钮 6/6 反模式;用户答 "需要" 同意进入 proposal | `3ae00fd`(⚠race)+ `21b7166`(P0-2A SPI + P0-2B 2 endpoint)+ `5c01814`(P0-4 AiButton 9 处 + P0-5 错态)+ `656a6a4`(⚠race P0-2C 11 文件)+ `5f93f77`(P0-3A+B 后端 30 文件 11 case)+ `9467bd1`(P0-3C 前端 4 文件)+ 3 docs commit;贯通度 15→60%;待 proposal 0030 升级 hook 硬拦 race add-all | 2026-05-28 → 06-25 |
...
| [0030](0030-race-add-all-hard-block.md) | 并行 session race add-all 从 nudge 升级到硬拦 — session-guard.sh bulk add + dirty>=1 默认 exit 2 + `CLAUDE_BULK_OK="<reason>"` 显式后门 + `CLAUDE_BYPASS_SESSION_GUARD=1` 一次性绕过 | **draft → implementing**(随本 commit 落 Step 1 实装,等 review 转 merged)| 2026-05-28 | 2026-05-28 epic 0028 单日 race 事故 2 次复发:`3ae00fd` 偷 P0-1 22 文件 + `656a6a4` 偷 P0-2C 11 文件;proposal 0008 留 nudge 已证失效 | _(本 commit:Step 1 session-guard.sh 升级 + 5 测试自检全绿;Step 2-5 文档同步随后跟)_ | _(2026-05-28 → 06-25,merged 后 4 周)_ |
