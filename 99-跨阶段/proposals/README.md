...
| [0039](0039-test-baseline-drift-auto-sync.md) | 测试基线漂移自动同步 — `mvn test` / `vitest run` 实测 vs 测试计划文档 case 差 > 10% 即 Stop hook nudge 提示同步(防止 Phase 04 用旧基线变相放低门槛)| **draft**(2026-05-29 起草,⚠ preemptive — 1 次本期事故 +21%/+327% 漂移 + 5+ 次历史手工同步 = 勉强 fast-track 边缘;fail-safe 1 周 0 触发 → rejected)| 2026-05-29 | 自全模块验收会话:实测 后端 868→1055 / 前端 117→500 与文档脱节 6 小时;`coverage_gap = 文档同步问题`;**自评 7.1/10 Approve with comments** | _(待 Wjl review)_ | _(待 merged + 4 周)_ |
| [0040](0040-phase05-gate-env-secrets-evidence.md) | Phase 05 上线 Gate §C 凭据章节加 "F-001 JWT_SECRET / F-002 Druid / F-003 REDIS_PASSWORD env 注入实证截图" 必填 3 行;不允许 N/A,豁免须走 proposal | **draft**(2026-05-29 起草,P1 — 安全审计 3 P1 finding 闭环路径)| 2026-05-29 | 自全模块验收 L6 安全审计:3 个 P1 默认值在生产漏 env 风险;[安全审计.md v0.2 §5 一票否决项](../../04-测试/安全审计.md) 已硬化但未绑 Gate;**自评 8.3/10 Approve** | _(待 Wjl review;首次 prod 上线时硬触发)_ | _(待 merged + 4 周 + 首次 prod 上线追 4 周)_ |
| [0041](0041-monthly-dep-audit-automation.md) | 月度依赖扫描自动化 — `.github/workflows/dep-audit.yml`(cron 每月 1 号)+ npm audit 临时切 npmjs.org 解决 npmmirror 限制 + jjwt < 0.12 / Spring Boot 落后 detect + 自动开 issue 归档 | **draft**(2026-05-29 起草,P2 — 老依赖 7 个月 0 触发升级 + npmmirror 镜像不支持 audit)| 2026-05-29 | 自全模块验收 L6 安全审计 F-004(jjwt 0.9.1 2018 老版本 CVE-2022-0840)+ F-005(npm audit 404);**自评 8.1/10 Approve** | _(待 Wjl review)_ | _(待 merged + 4 周 + 6 月 1 日首跑后 4 周)_ |
| [0042](0042-test-orchestrate-skill-full-regression-branch.md) | `plm-test-orchestrate` skill Step 1 加 "全模块复跑验收" 范围行 + Step 2 金字塔加 L5 性能 / L6 安全 2 层 + L4 允许 7 日内 commit 证据引用(明示标注)+ Phase 03→04 准入仍走全实跑 | **draft**(2026-05-29 起草,P3 — 把本期实际做法硬化)| 2026-05-29 | 自全模块验收 [测试报告 §2/§4.4/§7](../../04-测试/测试报告-2026-05-29-全模块验收.md)+ [复盘 §7.1 skill 回退](../../04-测试/测试复盘-2026-05-29.md);**自评 8.1/10 Approve** | _(待 Wjl review;下次"全模块复跑"场景验证)_ | _(待 merged + 4 周)_ |
