...
| F2.1 | 设计 | `requirement` | `requirements.html` | `tb_requirement` | `REQ-YYYY-NNNN` (ADR-0002) | 🟡 字段表已提案 / 代码待对齐 |
...
- 🟡 字段表已提案 / 代码待对齐 = **1 个**: requirement (本次,ADR-A 4 态实用版 + 加 aiValue 字段)
- 🟡 空壳 = **0 个**
- 🔴 空 = **0 个**

**审计待办** (跑偏检测 §M.6):
- ✅ project (本次完成): 5 处字段 drift 修复 + 双状态机 + 字典补全 (commit `20b5bb6`→`3c10238`→`522b6df`)
...
**审计结果摘要**:
- 🔴 严重 1 个: **Requirement (F2.1)** — 缺 `aiValue` + 状态机 4 vs 6 态分歧 → 建议下一轮先攻
- 🟡 中等 5 个: TestCase / Defect / Document / Submission / Release — 字典口径 / 状态机偏差 / 字段命名
- 🟢 轻微 6 个: Sprint / Task / TestPlan / TestReport / ApiDoc / ManualProduct — 字段层面已对齐,仅需补 §2 字段表
...
> **状态**: 提案中 — 此字段表是规则 §M.2 "先字段表后代码" 的产物,对应代码 commit 落地后,§1 中 Requirement 行从「🟢 已对齐」升级为「🟢 **PRD-aligned**」。
