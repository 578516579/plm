...
**统计**:
- 🟢 已对齐 = **17 个** (含 inception + prd + competitive,P0 闭环 + ued P1 启动)
- 🟡 空壳 = **13 个**
- 🔴 空 = **0 个**
...
| `title` | `title` | `uedVersions` 表 — 设计稿名称列 | varchar(200) | ✅ | 例: 农情大屏 v2 交互稿 |
| `version` | `version` | `uedVersions` 表 — 版本号列 | varchar(20) | ✅ | 默认 `v1.0` |
| `figmaUrl` | `figma_url` | `openFigmaSync()` + `modal-designpreview` canvas | varchar(500) | ❌ | Figma 文件链接 |
| `figmaSyncedAt` | `figma_synced_at` | `openFigmaSync()` 同步操作完成时间 | DATETIME | ❌ | 最近 Figma 同步时间 |
| `aiReviewResult` | `ai_review_result` | `uedReview` div — AI 评审报告 | LONGTEXT | ❌ | Markdown,`runUEDCheck()` 生成 |
| `aiReviewScore` | `ai_review_score` | (服务计算) | DECIMAL(5,2) | ❌ | 0-100,≥80 视为合格,`runUEDCheckFromModal()` |
| `aiReviewedAt` | `ai_reviewed_at` | (服务计算) | DATETIME | ❌ | AI 评审时间 |
| `aiGenerated` | `ai_generated` | (服务计算) | CHAR(1) | (default 'N') | `runUEDCheck()` 触发后置 'Y' |
| `status` | `status` | `uedVersions` 表 — 状态列 (`.b.bg/.bam/.bgr`) | varchar(2) | ✅ | 见 §3 |
| `designerUserId` | `designer_user_id` | (设计师/创建人) | BIGINT(20) | ✅ | sys_user FK |
| `reviewerUserId` | `reviewer_user_id` | (评审人) | BIGINT(20) | ❌ | 01→02 转态时填 |
| `createBy` etc. | (BaseEntity) | — | — | — | RuoYi 标准审计字段 |

**状态机** (4 态,源: 原型 ued.html 设计版本 badge `.b.bg/.bam/.bgr` + PRD §F2.3 "设计规范检查"):
- `00` 草稿 → `{01}`
- `01` 评审中 → `{00, 02}` (反向边:评审打回)
- `02` 已确认 → `{03}` (终态可废弃)
- `03` 已废弃 → `{}` (终态)

**AI 入口**: `POST /business/ued/ai/review/{id}` — 调用 §2.3 `ued-review-flow` (本期 mock:返回设计规范检查报告 + `aiReviewScore` 固定 88.0)。
