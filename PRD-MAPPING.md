...
| F4.6 | 质量 | `defect` | `defects.html` | `tb_defect` | `DEFECT-YYYY-NNNN` (ADR-0005) | 🟢 **PRD-aligned** |
...
| F5.5 | 文档 | `document` | (并入文档中心) | `tb_document` | `DOC-<TYPE>-YYYY-NNNN` (ADR-0007) | 🟢 **PRD-aligned** (归档/查询);AI 语义检索 → [路线图剥离清单](PLM-路线图.md) |
...
- 🟢 **PRD-aligned** (粗体,§2 字段表 + 代码 + 测试绿) = **31 个**: inception / **project** / **requirement** / competitive / prd / ued / arch / dbdesign / apidesign / **sprint** / **task** / **testcase** / **testplan** / **testreport** / testdata / autotest / **submission** / **defect** / **document** / **manual-product** / manual-impl / manual-ops / **apidoc** / analytics / dashboard / ai-agent / openspec / pipeline / feature-flag / dora / **release**
- 🟢 早期对齐 = **0 个** 🎉
- 🟡 字段表已提案 / 代码待对齐 = **0 个**
- 🟡 空壳 = **0 个**
- 🔴 空 = **0 个**
...
- ✅ Submission (完成): biz_submission_environment 5 值字典 + Service 白名单 + §2 字段表 (commit `ae77e61`)
- ✅ TestCase (完成,ADR-B Option B): category 字典 8 值字符串编码 + 白名单 + §2 字段表 (commit `9baac4c`→`534c67e`→`7c83e11`)
- ✅ Defect (完成,ADR-D Option A): 状态机 5→4 主态 + 反向边 03→00 + module 字段 (commit `657c8a8`→`e0a9a21`→`e55fe63`)
- ✅ Document (完成,ADR-C Option B 调整版): §2 字段表 + ADR-C 决策记录 (Document 承担 F5.5 归档/查询;AI 语义检索维持路线图剥离)
...
### Defect (F4.6) — 缺陷管理 [defects.html L167 modal-newdefect + L329-345 modal-defectedit + PRD §F4.6 L367-371]

> **状态**: 🟢 **PRD-aligned** (字段表 commit `657c8a8` → 代码 commit `e0a9a21`,mvn compile 绿,proposal [0302 ADR-D Option A](99-跨阶段/proposals/0302-adr-d-defect-state-machine.md) 落地)

**PRD/原型 → 字段映射**:

| 字段 | 列名 | 原型来源 | 类型 | 必填 | 备注 |
|---|---|---|---|:--:|---|
| `defectId` | `defect_id` | (主键) | BIGINT(20) | ✅ | AUTO_INCREMENT |
| `defectNo` | `defect_no` | (业务编号) | varchar(32) | ✅ | `DEFECT-YYYY-NNNN` (ADR-0005) |
| `projectId` | `project_id` | (隐式 FK) | BIGINT(20) | ✅ | FK→tb_project.id |
| `sprintId` | `sprint_id` | (关联迭代) | BIGINT(20) | ❌ | FK→tb_sprint.id |
| `taskId` | `task_id` | (关联任务) | BIGINT(20) | ❌ | FK→tb_task.id |
| `title` | `title` | `缺陷标题 *` (`nd-title`) | varchar(200) | ✅ | — |
| `description` | `description` | (详细描述) | TEXT | ❌ | — |
| `severity` | `severity` | `严重级别` select (`nd-level` P0-P3) | varchar(2) | ✅ | 字典 `biz_defect_severity`:`00` P0 阻塞 / `01` P1 严重 / `02` P2 一般 / `03` P3 轻微 |
| `category` | `category` | (缺陷分类) | varchar(2) | ✅ | 字典 `biz_defect_category`:功能/性能/兼容性/安全/易用性/其他 |
| `status` | `status` | `状态` select (`dem-status` 4 选项) | varchar(2) | ✅ | 字典 `biz_defect_status` 4 态 + 反向边 (ADR-D Option A,见 §决策记录 D1) |
| `module` | `module` | `所属模块` (`dem-module`) | varchar(200) | ❌ | 本次新增字段 (D2) |
| `assigneeUserId` | `assignee_user_id` | `指派给` select (`nd-owner`) | BIGINT(20) | ❌ | FK→sys_user |
| `reporterUserId` | `reporter_user_id` | (报告人,默认当前登录用户) | BIGINT(20) | ✅ | FK→sys_user |
| `reproduceSteps` | `reproduce_steps` | `复现步骤` (`nd-steps`) | TEXT | ❌ | — |
| `expectedResult` | `expected_result` | (期望结果) | TEXT | ❌ | — |
| `actualResult` | `actual_result` | (实际结果) | TEXT | ❌ | — |
| `resolution` | `resolution` | `修复备注` (`dem-note`) | varchar(500) | 条件必填 | 进入 `02 待验证` 必填(705) |
| `tags` | `tags` | (标签 CSV,e.g. regression,flaky) | varchar(200) | ❌ | 承载非分类维度 |
| `delFlag` | `del_flag` | (软删除) | CHAR(1) | (auto) | — |

**字段决策记录**:
- **D1 = ADR-D (proposal 0302 Option A): 状态机重设为 4 主态 + 反向边 03→00**
  | dict_value | dict_label | 旧 dict_label | 转换 |
  |---|---|---|---|
  | `00` | 待确认 | 新建+已确认 (合并) | → {01 修复中, 03 已关闭} |
  | `01` | 修复中 | 处理中 (改名) | → {02 待验证, 00 待确认} (验证失败回退) |
  | `02` | 待验证 | 已解决 (改名,语义更准) | → {03 已关闭, 01 修复中} (验证失败回修复) |
  | `03` | 已关闭 | 已关闭 (保留) | → {00 待确认} (反向边:重开) |
  - 与原型 `dem-status` 4 选项 (待确认/修复中/待验证/已关闭) 100% 对齐
  - 状态机层面减少 1 个旧"已确认"态(实际工作中"新建后自动确认"是常见简化)
  - 数据迁移 (dev): 旧 '00 新建'/'01 已确认' → 新 '00 待确认';旧 '02 处理中' → '01 修复中';旧 '03 已解决' → '02 待验证';旧 '04 已关闭' → '03 已关闭'
- **D2: 新增 `module` 字段** — 原型 modal-defectedit `dem-module` "所属模块" 输入框,值 = 业务模块名 (e.g. "项目管理" / "需求评审");自由文本 varchar(200),不字典化(各项目模块名不同)
- **D3: `resolution` 必填触发条件由"进入 03 已解决"改为"进入 02 待验证"** — 因为状态机重设后,"修复完成转待验证"是 resolution 真正需要的节点
- **D4: 705 错误码语义保持** — 进入待验证未填解决说明仍抛 705

**状态机** (4 主态 + 反向边 03→00,ADR-D):
- `00` 待确认 → `{01, 03}` (转入修复 / 直接关闭如重复)
- `01` 修复中 → `{02, 00}` (修复完转验证 / 验证失败退回待确认)
- `02` 待验证 → `{03, 01}` (验证通过关闭 / 验证失败回修复)
- `03` 已关闭 → `{00}` (反向边:重开,Severity 升级或客户重报)

非法转换抛 ServiceException(601);进入 `02` 缺 resolution 抛 705。

### Document (F5.5 部分) — 文档中心 (合并 5 stub) + 承担 F5.5 归档能力

> **状态**: 🟢 **PRD-aligned** (字段表本次补,proposal [0301 ADR-C](99-跨阶段/proposals/0301-adr-c-document-vs-knowledge-base.md) 阶段 1 落地)
>
> **重要 ADR 决策(ADR-C 与路线图协调)**:
> - **当前 `plm-document` 承担 PRD §F5.5 "知识库管理" 的归档 + 查询 + 复用 引用** 能力
> - **PRD §F5.5 的 AI 语义检索 + Milvus 向量库 + 跨项目语义复用推荐** 被列入 [PLM-路线图.md "永不做剥离清单" 第 1 项 "AgriKB + Milvus 向量"](PLM-路线图.md)
> - 所以 proposal 0301 Option B 阶段 2 (拆出 `plm-knowledge-base` 模块) 与路线图冲突,**superseded 为接受现状 + 标注 F5.5 高级能力剥离**

**PRD/原型 → 字段映射** (Document 无独立原型 HTML,字段来源于合并 5 stub 的设计 + ADR-0007 文档编号规则):

| 字段 | 列名 | 来源 | 类型 | 必填 | 备注 |
|---|---|---|---|:--:|---|
| `documentId` | `document_id` | (主键) | BIGINT(20) | ✅ | AUTO_INCREMENT |
| `documentNo` | `document_no` | (业务编号) | varchar(32) | ✅ | `DOC-<TYPE>-YYYY-NNNN` (ADR-0007) |
| `projectId` | `project_id` | (隐式 FK) | BIGINT(20) | ✅ | FK→tb_project.id |
| `relatedEntityType` | `related_entity_type` | (多态 FK 关联类型) | varchar(20) | ❌ | e.g. "requirement" / "task" / "release" |
| `relatedEntityId` | `related_entity_id` | (多态 FK 关联 ID) | BIGINT(20) | ❌ | 与 relatedEntityType 配对 |
| `docType` | `doc_type` | (合并 5 stub 的鉴别字段) | varchar(20) | ✅ | 字典 `biz_doc_type` 12 值:prd/arch/db_design/api_design/proposal/ued/test_plan/test_report/api_doc/manual_product/manual_impl/manual_ops |
| `title` | `title` | (文档标题) | varchar(200) | ✅ | — |
| `content` | `content` | (Markdown 全文) | LONGTEXT | ❌ | — |
| `version` | `version` | (版本号) | varchar(20) | ✅ | 默认 `v1.0` |
| `status` | `status` | (状态机) | varchar(2) | ✅ | 字典 `biz_doc_status` 4 态含反向边 |
| `authorUserId` | `author_user_id` | (作者) | BIGINT(20) | ✅ | FK→sys_user |
| `reviewerUserId` | `reviewer_user_id` | (审核人) | BIGINT(20) | ❌ | FK→sys_user |
| `tags` | `tags` | (CSV 标签) | varchar(200) | ❌ | 用于"知识库归档"分类承载 |
| `delFlag` | `del_flag` | (软删除) | CHAR(1) | (auto) | — |

**字段决策记录**:
- **D1 = ADR-C (proposal 0301 Option B 调整版)**: Document 模块同时承担两个角色:
  - 角色 1: **通用文档中心**(prd/arch/dbdesign/apidesign/proposal 5 stub 的合并存储,通过 `docType` 鉴别)— 这是模块的原始设计
  - 角色 2: **PRD §F5.5 知识库的归档 + 查询 + 复用 引用部分** — 通过 `tags` + `docType` 实现分类与检索,通过 `relatedEntityType`/`relatedEntityId` 实现跨项目引用追溯
  - **PRD §F5.5 未承担**: AI 语义检索 / Milvus 向量库 / embedding 字段 / 复用推荐评分 — 这些在 [PLM-路线图.md "永不做剥离清单"](PLM-路线图.md) 第 1 项明确剥离 (项目当前 early/solo/internal-tool 模式)
- **D2: docType 字典 12 值的设计意义** — 在 v0.5+ 远期 backlog 兑现时,如果重新启用 AgriKB,可加 `docType='knowledge_article'` 作为知识条目类型,Document 模块以此覆盖更广。但本期不预留。
- **D3: `tags` 字段承担"知识库归档分类"角色** — e.g. `tags='best-practice,灌溉,sprint-22'` 既是标签,也是归档维度
- **D4: 文档编号 `DOC-<TYPE>-YYYY-NNNN`** — `<TYPE>` 来自 docType,例 `DOC-PRD-2026-0001`,这是 ADR-0007 规则

**状态机** (4 态含反向边):
- `00` 草稿 → `{01}`
- `01` 待评审 → `{00, 02}` (评审打回 / 通过)
- `02` 已发布 → `{01, 03}` (重新评审 / 归档)
- `03` 已归档 → `{}` (终态)
...
| `defect` | **F4.6** | `00 待确认 → {01,03}` `01 修复中 → {00,02}` `02 待验证 → {01,03}` `03 已关闭 → {00}` (4 态 + 重开反向边) | ADR-D Option A;旧 "6 态含重开" 描述已废弃,实际 4 态;进入 02 必填 resolution(705) |
