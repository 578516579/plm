...
- 🟢 **PRD-aligned** (粗体,§2 字段表 + 代码 + 测试绿) = **26 个**: inception / **project** / **requirement** / competitive / prd / ued / arch / dbdesign / apidesign / **sprint** / **task** / **testplan** / **testreport** / testdata / autotest / **manual-product** / manual-impl / manual-ops / **apidoc** / analytics / dashboard / ai-agent / openspec / pipeline / feature-flag / dora
- 🟢 早期对齐 (代码已跑通,§2 字段表待补,drift 审计 🟡 中等) = **5 个**: testcase / defect / document / submission / release
- 🟡 字段表已提案 / 代码待对齐 = **0 个**
- 🟡 空壳 = **0 个**
- 🔴 空 = **0 个**

**注 (SSoT 矛盾披露)**: §1 大表中 `submission` (F4.4) / `release` (扩展) 两行历史标记为 🟢 **PRD-aligned** 粗体,但 §2 字段表实际缺失;审计已识别 (🟡 中等,environment 字典口径 / strategy 字典缺 direct_replace)。本次未一并修正以缩小 commit 范围,留下一轮专项处理。
...
- ✅ 6 个 🟢 轻微模块补 §2 字段表 (本 commit): Sprint / Task / TestPlan / TestReport / ApiDoc / ManualProduct
...
**审计结果摘要 (剩余 5 个 🟡 中等待修)**:
- TestCase (F4.2): category 字典缺农业专项类型 (需 ADR)
- Defect (F4.6): 状态机三方不一致 (PRD 5 态 / 原型 4 态 / Domain 5 态 label 不同 / MAPPING §3 标 6 态);`module` 字段缺
- Document (F5.5): 与 PRD §F5.5 "知识库管理" 概念错位,需 ADR-C 决策 (重命名 vs 拆出 knowledge-base 模块)
- Submission (F4.4): environment 字典口径不统一 (原型 test/pre vs SQL dev/staging/prod)
- Release (扩展): strategy 字典缺 `direct_replace` 选项
