...
**统计**:
- 🟢 已对齐 = **22 个** (含 inception + prd + competitive + ued + arch + dbdesign + apidesign + testdata + autotest,P0/P1 闭环)
- 🟡 空壳 = **9 个** (manual-impl/manual-ops/analytics/dashboard/ai-agent/openspec/pipeline/feature-flag/dora)
- 🔴 空 = **0 个**
...
### UED (F2.3) — UED 设计协同 [ued.html + PRD §F2.3]

| 字段 | 列名 | 原型来源 (ued.html) | 类型 | 必填 | 备注 |
|---|---|---|---|:--:|---|
| `uedId` | `ued_id` | (主键) | BIGINT(20) | ✅ | AUTO_INCREMENT |
| `uedNo` | `ued_no` | (业务编号) | varchar(32) | ✅ | `UED-YYYY-NNNN` |
| `projectId` | `project_id` | (关联项目) | BIGINT(20) | ✅ | FK → tb_project |
| `title` | `title` | `uedVersions` 表 — 设计稿名称 | varchar(200) | ✅ | 例: 农情大屏设计 v2 |
| `designType` | `design_type` | 农业UI组件库类型卡片 | varchar(20) | ❌ | 字典 `biz_ued_design_type`: ue/ui/motion/icon |
| `platform` | `platform` | 农业UI组件库场景 (大屏/移动端/IoT) | varchar(20) | ❌ | 字典 `biz_ued_platform`: web/mobile/iot/miniapp |
| `figmaFileKey` | `figma_file_key` | `openFigmaSync()` Figma 同步 | varchar(200) | ❌ | Figma 文件 Key (用于 MCP 同步) |
| `figmaUrl` | `figma_url` | `📎 从Figma同步设计稿` | varchar(500) | ❌ | Figma 链接 |
| `version` | `version` | `dp-meta` (设计稿预览 modal 元信息) | varchar(20) | ❌ | 默认 `v1.0` |
| `description` | `description` | 设计说明 | TEXT | ❌ | 设计背景与目标 |
| `reviewReport` | `review_report` | `uedReview` — AI设计评审报告 | LONGTEXT | ❌ | AI 生成规范检查报告 Markdown |
| `complianceScore` | `compliance_score` | `runUEDCheck()` 评审结果 | DECIMAL(5,2) | ❌ | 0-100 规范遵从度,≥80 通过 |
| `aiGenerated` | `ai_generated` | (服务计算) | CHAR(1) | (default 'N') | `runUEDCheck` 触发后置 'Y' |
| `aiGeneratedAt` | `ai_generated_at` | (服务计算) | DATETIME | ❌ | AI 评审时间 |
| `requirementId` | `requirement_id` | (双向关联需求,PRD §F2.3) | BIGINT(20) | ❌ | FK → tb_requirement (可选) |
| `status` | `status` | (状态徽章) | varchar(2) | ✅ | 见 §3 |
| `designerUserId` | `designer_user_id` | (设计师) | BIGINT(20) | ✅ | sys_user FK |
| `reviewerUserId` | `reviewer_user_id` | (评审人) | BIGINT(20) | ❌ | 01→02 转态时填 |
| `createBy` etc. | (BaseEntity) | — | — | — | RuoYi 标准 |

**状态机** (3 态 + 1 终态,含反向边 01→00,与 prd 模块同构):
- `00` 草稿 → `{01}`
- `01` 评审中 → `{00, 02}` (反向边: 评审打回草稿)
- `02` 已确认 → `{03}`
- `03` 已废弃 → `{}` (终态)

**AI 入口**: `POST /business/ued/ai/review/{id}` — 调用 §2.3 `ued-review-flow` (本期 mock: 检查设计规范遵从度,complianceScore 固定 88.0 ≥80)。

### Arch (F3.1) — 系统架构设计 [archdesign.html + PRD §F3.1]
...
| `projectId` | `project_id` | (关联项目) | BIGINT(20) | ✅ | FK → tb_project |
| `title` | `title` | 架构方案标题 | varchar(200) | ✅ | — |
| `archMode` | `arch_mode` | `arch-mode` select | varchar(30) | ❌ | 字典 `biz_arch_mode`: microservice/monolith/serverless/layered |
| `techStack` | `tech_stack` | `arch-lang` select | varchar(50) | ❌ | 字典 `biz_arch_tech`: java_springboot3/go_gin/python_fastapi/nodejs |
| `dbStack` | `db_stack` | `arch-db` select | varchar(50) | ❌ | 字典 `biz_arch_db`: postgresql_redis/mysql_redis/kdb |
| `aiOrchestration` | `ai_orchestration` | `arch-ai` select | varchar(50) | ❌ | 字典 `biz_arch_ai`: dify_deepseek/dify_chatglm/langchain |
| `deployMode` | `deploy_mode` | `arch-deploy` select | varchar(30) | ❌ | 字典 `biz_arch_deploy`: kubernetes/docker_compose/baremetal |
| `iotProtocol` | `iot_protocol` | `arch-iot` select | varchar(30) | ❌ | 字典 `biz_arch_iot`: mqtt_emqx/http_polling/websocket |
| `archContent` | `arch_content` | `archContent` div (架构方案 Markdown) | LONGTEXT | ❌ | 架构文档正文 |
| `c4Diagram` | `c4_diagram` | `archDiagram` div (C4 容器图) | LONGTEXT | ❌ | Mermaid/PlantUML 源码 |
| `nfrContent` | `nfr_content` | `archNFR` card (非功能需求) | TEXT | ❌ | — |
| `reviewReport` | `review_report` | AI 架构推荐报告 | LONGTEXT | ❌ | AI 生成 Markdown |
| `aiGenerated` | `ai_generated` | (服务计算) | CHAR(1) | (default 'N') | aiRecommend 触发后置 'Y' |
| `aiGeneratedAt` | `ai_generated_at` | (服务计算) | DATETIME | ❌ | 生成时间 |
| `status` | `status` | (状态机) | varchar(2) | ✅ | 见 §3 |
| `authorUserId` | `author_user_id` | (方案作者) | BIGINT(20) | ✅ | sys_user FK |
...
**AI 入口**: `POST /business/arch/ai/recommend/{id}` — 调用 §F3.1 `arch-design-flow` (本期 mock: 6 维度架构推荐报告)。

### Dbdesign (F3.2) — 数据库设计 [dbdesign.html + PRD §F3.2]
...
| `projectId` | `project_id` | (关联项目) | BIGINT(20) | ✅ | FK → tb_project |
| `title` | `title` | 数据库设计标题 | varchar(200) | ✅ | — |
| `dbType` | `db_type` | 数据库类型 select | varchar(20) | ❌ | 字典 `biz_dbdesign_type`: mysql/postgresql/kdb/sqlite |
| `erContent` | `er_content` | `erDiagram` div (ER 图) | LONGTEXT | ❌ | Mermaid/PlantUML 源码 |
| `dictContent` | `dict_content` | `dbDict` div (数据字典) | LONGTEXT | ❌ | Markdown 表格 |
| `ddlContent` | `ddl_content` | `dbSql` div (建表 SQL) | LONGTEXT | ❌ | DDL SQL 全文 |
| `reviewReport` | `review_report` | AI ER 审查报告 | LONGTEXT | ❌ | AI 生成 Markdown |
| `aiGenerated` | `ai_generated` | (服务计算) | CHAR(1) | (default 'N') | aiEr 触发后置 'Y' |
| `aiGeneratedAt` | `ai_generated_at` | (服务计算) | DATETIME | ❌ | 生成时间 |
| `status` | `status` | (状态机) | varchar(2) | ✅ | 见 §3 |
| `authorUserId` | `author_user_id` | (设计者) | BIGINT(20) | ✅ | sys_user FK |
| `reviewerUserId` | `reviewer_user_id` | (评审人) | BIGINT(20) | ❌ | — |
| `createBy` etc. | (BaseEntity) | — | — | — | RuoYi 标准 |
...
**AI 入口**: `POST /business/dbdesign/ai/er/{id}` — 调用 §F3.2 `db-design-flow` (本期 mock: 5 维度 ER 图审查报告)。

### Apidesign (F3.3) — 接口详细设计 [apidesign.html + PRD §F3.3]

| 字段 | 列名 | 原型来源 (apidesign.html) | 类型 | 必填 | 备注 |
|---|---|---|---|:--:|---|
| `apidesignId` | `apidesign_id` | (主键) | BIGINT(20) | ✅ | AUTO_INCREMENT |
| `apidesignNo` | `apidesign_no` | (业务编号) | varchar(32) | ✅ | `APID-YYYY-NNNN` |
| `projectId` | `project_id` | (关联项目) | BIGINT(20) | ✅ | FK → tb_project |
| `title` | `title` | 接口设计标题 | varchar(200) | ✅ | — |
| `httpMethod` | `http_method` | `na-method` select (modal-newapi) | varchar(10) | ❌ | 字典 `biz_apidesign_method`: get/post/put/delete/patch |
| `apiPath` | `api_path` | `na-path` input (modal-newapi) | varchar(500) | ❌ | 接口路径 |
| `description` | `description` | `na-desc` input (modal-newapi) | TEXT | ❌ | 接口功能描述 |
| `requestSchema` | `request_schema` | 请求参数 JSON Schema | LONGTEXT | ❌ | — |
| `responseSchema` | `response_schema` | 响应参数 JSON Schema | LONGTEXT | ❌ | — |
| `errorCodes` | `error_codes` | 错误码说明 | TEXT | ❌ | — |
| `openapiContent` | `openapi_content` | OpenAPI 规范 YAML | LONGTEXT | ❌ | AI 生成 |
| `mockEnabled` | `mock_enabled` | Mock 服务控制台开关 | CHAR(1) | (default 'N') | Y=开启 Mock |
| `version` | `version` | 接口版本 | varchar(20) | ❌ | 默认 `v1.0` |
| `reviewReport` | `review_report` | AI OpenAPI 报告 | LONGTEXT | ❌ | AI 生成 Markdown |
| `aiGenerated` | `ai_generated` | (服务计算) | CHAR(1) | (default 'N') | aiOpenapi 触发后置 'Y' |
| `aiGeneratedAt` | `ai_generated_at` | (服务计算) | DATETIME | ❌ | 生成时间 |
| `status` | `status` | (状态机) | varchar(2) | ✅ | 见 §3 |
| `authorUserId` | `author_user_id` | (设计者) | BIGINT(20) | ✅ | sys_user FK |
| `reviewerUserId` | `reviewer_user_id` | (评审人) | BIGINT(20) | ❌ | — |
...
**AI 入口**: `POST /business/apidesign/ai/openapi/{id}` — 调用 §F3.3 `detail-design-flow` (本期 mock: RESTful 规范检查 + OpenAPI 3.1 摘要生成)。

### Testdata (F4.3) — 测试数据工厂 [testdata.html + PRD §F4.3]

| 字段 | 列名 | 原型来源 (testdata.html) | 类型 | 必填 | 备注 |
|---|---|---|---|:--:|---|
| `testdataId` | `testdata_id` | (主键) | BIGINT(20) | ✅ | AUTO_INCREMENT |
| `testdataNo` | `testdata_no` | (业务编号) | varchar(32) | ✅ | `TD-YYYY-NNNN` |
| `projectId` | `project_id` | (关联项目) | BIGINT(20) | ✅ | FK → tb_project |
| `title` | `title` | 数据集名称 | varchar(200) | ✅ | 例: 土壤传感器数据集 v1 |
| `targetTable` | `target_table` | `td-table` 下拉 | varchar(60) | ❌ | 字典 `biz_testdata_table` |
| `generateCount` | `generate_count` | `td-count` 输入框 | INT | ❌ | 默认 1000 |
| `outputFormat` | `output_format` | `td-format` 单选 | varchar(20) | ❌ | 字典 `biz_testdata_format`: json/sql_insert/csv; 默认 json |
| `ruleCoordinate` | `rule_coordinate` | `rule-coord` 复选框 | char(1) | ❌ | Y/N; 默认 Y |
| `ruleTimeSeries` | `rule_time_series` | `rule-ts` 复选框 | char(1) | ❌ | Y/N; 默认 Y |
| `ruleSensorRange` | `rule_sensor_range` | `rule-sensor` 复选框 | char(1) | ❌ | Y/N; 默认 Y |
| `ruleIncludeAbnormal` | `rule_include_abnormal` | `rule-abnormal` 复选框 | char(1) | ❌ | Y/N; 默认 N |
| `generatedData` | `generated_data` | `tdPreview` 预览区 | LONGTEXT | ❌ | AI 生成的数据预览 |
| `writeTarget` | `write_target` | `modal-db-write` 目标环境 | varchar(10) | ❌ | 字典 `biz_testdata_target`: test/dev |
| `writeMode` | `write_mode` | `modal-db-write` 写入模式 | varchar(10) | ❌ | 字典 `biz_testdata_write_mode`: append/truncate/upsert |
| `aiGenerated` | `ai_generated` | (服务计算) | char(1) | ✅ | Y/N |
| `aiGeneratedAt` | `ai_generated_at` | (服务计算) | DATETIME | ❌ | AI 生成时间 |
| `status` | `status` | 状态栏 | char(2) | ✅ | 字典 `biz_testdata_status`: 00草稿/01生成中/02已完成/03已废弃 |
| `authorUserId` | `author_user_id` | 创建者 | BIGINT(20) | ✅ | FK → sys_user |

状态机: `00→{01,02}` `01→{02,03}` `02→{03}` `03→{}` (终态)

**AI 入口**: `POST /business/testdata/ai/generate/{id}` — 调用 §F4.3 `data-gen-flow` (本期 mock: 按 targetTable + outputFormat 生成 JSON/SQL INSERT/CSV 预览数据，ruleCoordinate/ruleTimeSeries/ruleSensorRange/ruleIncludeAbnormal 字段写入规则摘要)。

### Autotest (F4.5) — 自动化测试套件 [autotest.html + PRD §F4.5]

| 字段 | 列名 | 原型来源 (autotest.html) | 类型 | 必填 | 备注 |
|---|---|---|---|:--:|---|
| `autotestId` | `autotest_id` | (主键) | BIGINT(20) | ✅ | AUTO_INCREMENT |
| `autotestNo` | `autotest_no` | (业务编号) | varchar(32) | ✅ | `AT-YYYY-NNNN` |
| `projectId` | `project_id` | (关联项目) | BIGINT(20) | ✅ | FK → tb_project |
| `title` | `title` | `autotestTable` 套件名称列 | varchar(200) | ✅ | 例: 农情接口自动化 |
| `suiteType` | `suite_type` | 套件类型 (api/e2e/unit/performance) | varchar(20) | ❌ | 字典 `biz_autotest_type` |
| `framework` | `framework` | 框架 (playwright/pytest/jest/jmeter) | varchar(30) | ❌ | 字典 `biz_autotest_framework` |
| `scriptContent` | `script_content` | AI 脚本生成区 | LONGTEXT | ❌ | AI 生成的测试脚本 |
| `lastRunAt` | `last_run_at` | `autotestTable` 最近执行时间列 | DATETIME | ❌ | 最近执行时间 |
| `lastRunResult` | `last_run_result` | `autotestTable` 结果列 | varchar(10) | ❌ | 字典 `biz_autotest_result`: passed/failed/error |
| `passRate` | `pass_rate` | `autotestTable` 通过率列 | DECIMAL(5,2) | ❌ | 0-100 |
| `totalCases` | `total_cases` | (运行统计) | INT | ❌ | 默认 0 |
| `failedCases` | `failed_cases` | `autotestTable` 失败用例列 | INT | ❌ | 默认 0 |
| `executionTime` | `execution_time` | `autotestTable` 执行耗时列 (ms) | INT | ❌ | 默认 0 |
| `scheduleCron` | `schedule_cron` | 定时调度 Cron 表达式 | varchar(50) | ❌ | 例: `0 0 2 * * ?` |
| `aiGenerated` | `ai_generated` | (服务计算) | char(1) | ✅ | Y/N |
| `aiGeneratedAt` | `ai_generated_at` | (服务计算) | DATETIME | ❌ | AI 脚本生成时间 |
| `status` | `status` | 状态 | char(2) | ✅ | 字典 `biz_autotest_status`: 00草稿/01已激活/02已归档 |
| `authorUserId` | `author_user_id` | 创建者 | BIGINT(20) | ✅ | FK → sys_user |

状态机: `00→{01}` `01→{00,02}` `02→{}` (终态; 00←01 反向=停用)

**AI 入口**: `POST /business/autotest/ai/script/{id}` — 调用 §F4.5 `auto-test-flow` (本期 mock: 按 framework 字段生成 Playwright TypeScript 或 pytest Python 测试脚本骨架，内含农情平台接口用例)。
...
| `ued` | **F2.3** | `00→01→{00,02}` `02→{03}` `03` 终态 | 01→00 评审打回 |
| `arch` | **F3.1** | `00→01→{00,02}` `02→{03}` `03` 终态 | 01→00 评审打回 |
| `dbdesign` | **F3.2** | `00→01→{00,02}` `02→{03}` `03` 终态 | 01→00 评审打回 |
| `apidesign` | **F3.3** | `00→01→{00,02}` `02→{03}` `03` 终态 | 01→00 评审打回 |
...
| `testdata` | **F4.3** | `00→{01,02}` `01→{02,03}` `02→{03}` `03` 终态 | 可跳过生成中直接完成 |
| `autotest` | **F4.5** | `00→{01}` `01→{00,02}` `02` 终态 | 01→00 停用反向边 |
...
| F3.1 | `arch-design-flow` | arch | `POST /business/arch/ai/recommend` | 🟡 mock 已实现 |
| F3.2 | `db-design-flow` | dbdesign | `POST /business/dbdesign/ai/er` | 🟡 mock 已实现 |
| F3.3 | `detail-design-flow` | apidesign | `POST /business/apidesign/ai/openapi` | 🟡 mock 已实现 |
...
| F4.3 | `data-gen-flow` | testdata | `POST /business/testdata/ai/generate/{id}` | 🟡 mock 已实现 |
| F4.5 | `auto-test-flow` | autotest | `POST /business/autotest/ai/script/{id}` | 🟡 mock 已实现 |
...
## 7. 空壳模块的 PRD-align 实施优先级 (路线图)

按 PRD §3.5 迭代规划 (Phase 1~4) 优先级排序:

### ✅ P0 (Phase 1 MVP 范围) — 全部完成
1. ✅ `inception` (F1.1) — 立项 AI 助手
...
plm-admin (entry)
  ...
  └── 业务模块 × 30
       ...
       │    apidoc, manual-product, inception, competitive, prd,
       │    ued, arch, dbdesign, apidesign, testdata, autotest
       └── (空壳 9,待 PRD-align)
            manual-impl, manual-ops,
