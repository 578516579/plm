...
| F1.2 | 立项 | `project` | `projects.html` | `tb_project` | `PRJ-YYYY-NNNN` (ADR-0001) | 🟡 字段表已提案 / 代码待对齐 |
...
- 🟢 **PRD-aligned** (粗体,§2 字段表 + 代码) = **18 个**: inception / competitive / prd / ued / arch / dbdesign / apidesign / testdata / autotest / manual-impl / manual-ops / analytics / dashboard / ai-agent / openspec / pipeline / feature-flag / dora
...
- 🟡 字段表已提案 / 代码待对齐 = **1 个**: project (本次 commit 加 §2 字段表,代码 commit 待跟进)
- 🟡 空壳 = **0 个**
- 🔴 空 = **0 个**

**审计待办** (跑偏检测 §M.6):
- project 的 §2 字段表已发现 5 处字段 drift(缺 businessLine/priority/lifecyclePhase/progress/health,多 budget);**其余 12 个"早期对齐"模块极可能存在类似 drift**,需在 project 代码落地后启动专项审计(逐模块对比 原型 HTML 表单字段 / PRD §F 章节 / 当前 Domain.java)。
...
> **状态**: 提案中 — 此字段表是规则 §M.2 "先字段表后代码" 的产物;对应代码 commit 落地后,§1 中 Project 行从「🟢 已对齐」升级为「🟢 **PRD-aligned**」并把本块「提案中」标记移除。
