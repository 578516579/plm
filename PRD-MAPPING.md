...
- 🟢 **PRD-aligned** (粗体,§2 字段表 + 代码 + 测试绿) = **27 个**: inception / **project** / **requirement** / competitive / prd / ued / arch / dbdesign / apidesign / **sprint** / **task** / **testplan** / **testreport** / testdata / autotest / **manual-product** / manual-impl / manual-ops / **apidoc** / analytics / dashboard / ai-agent / openspec / pipeline / feature-flag / dora / **release**
- 🟢 早期对齐 (代码已跑通,§2 字段表待补,drift 审计 🟡 中等) = **4 个**: testcase / defect / document / submission
- 🟡 字段表已提案 / 代码待对齐 = **0 个**
- 🟡 空壳 = **0 个**
- 🔴 空 = **0 个**

**注 (SSoT 矛盾披露)**: §1 大表中 `submission` (F4.4) 行历史标记为 🟢 **PRD-aligned** 粗体,但 §2 字段表实际缺失;审计 🟡 中等 (environment 字典口径)。留下一轮专项处理。
...
- ✅ Release (完成): strategy 字典补 `direct_replace` + §2 字段表 (本 commit)
