...
| Java 包路径 | `cn.com.bosssfot.dv.plm.analytics` |
...
- [ ] 复用 `plm-project` 模板创建 Domain / Mapper / Service / Controller (放在 `analytics/` 包下)
- [ ] 走 Phase 01 → 02 → 03 Gate 完成立项 → 设计 → 开发
- [ ] DB 表 `tb_analytics` + 字典 + 菜单
- [ ] 前端 `plm-frontend/packages/plm-analytics/` 实现 api / types / views
