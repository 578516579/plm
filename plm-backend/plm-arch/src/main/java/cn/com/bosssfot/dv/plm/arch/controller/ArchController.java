...
/**
 * 系统概要设计 HLD Controller — PRD §F3.1
 * /business/arch/* (6 标准 + AI 生成入口)
 */
@RestController
...
public class ArchController extends BaseController {
    ...
    @Log(title = "概要设计", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public void export(HttpServletResponse response, Arch arch) {
        List<Arch> list = archService.selectArchList(arch);
        new ExcelUtil<Arch>(Arch.class).exportExcel(response, list, "概要设计数据");
    }
    ...
    @Log(title = "概要设计", businessType = BusinessType.INSERT)
    @PostMapping
    ...
    @Log(title = "概要设计", businessType = BusinessType.UPDATE)
    @PutMapping
    ...
    @Log(title = "概要设计", businessType = BusinessType.DELETE)
    @DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable Long[] ids) {
        return toAjax(archService.deleteArchByIds(ids));
    }

    /** PRD §F3.1 AI 生成架构方案 + C4 容器图 — arch-design-flow */
    @PreAuthorize("@ss.hasPermi('business:arch:edit')")
    @Log(title = "概要设计-AI生成", businessType = BusinessType.OTHER)
    @PostMapping("/ai/generate/{id}")
    public AjaxResult aiGenerate(@PathVariable("id") Long id) {
        return success(archService.aiGenerate(id));
    }
}
