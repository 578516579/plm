...
public class AutotestController extends BaseController {
    ...
    public void export(HttpServletResponse response, Autotest autotest) {
        ...
        new ExcelUtil<Autotest>(Autotest.class).exportExcel(response, list, "自动化测试套件数据");
    }
    ...
    /** PRD §F4.5 AI 脚本生成 */
    @PreAuthorize("@ss.hasPermi('business:autotest:edit')")
    @Log(title = "自动化测试-AI脚本", businessType = BusinessType.OTHER)
    @PostMapping("/ai/script/{id}")
    public AjaxResult aiScript(@PathVariable("id") Long id) {
        return success(autotestService.aiScript(id));
    }
}
