...
public class DashboardController extends BaseController {
    ...
    @Log(title = "工作台", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public void export(HttpServletResponse response, Dashboard dashboard) {
        ...
        new ExcelUtil<Dashboard>(Dashboard.class).exportExcel(response, list, "工作台数据");
    }
    ...
    @Log(title = "工作台", businessType = BusinessType.INSERT)
    @PostMapping
    ...
    @Log(title = "工作台", businessType = BusinessType.DELETE)
    @DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable Long[] ids) {
        return toAjax(dashboardService.deleteDashboardByIds(ids));
    }

    /** UI §4.2 工作台聚合查询 — 返回 6 类 widget 数据 */
    @PreAuthorize("@ss.hasPermi('business:dashboard:query')")
    @GetMapping("/aggregate")
    public AjaxResult aggregate(@RequestParam(value = "ownerUserId", required = false) Long ownerUserId) {
        return success(dashboardService.aggregate(ownerUserId));
    }
}
