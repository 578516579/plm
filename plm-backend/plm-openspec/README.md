...
| Java 包路径 | `cn.com.bosssfot.dv.plm.openspec` |
...
- [ ] 复用 `plm-project` 模板创建 Domain / Mapper / Service / Controller (放在 `openspec/` 包下)
- [ ] 走 Phase 01 → 02 → 03 Gate 完成立项 → 设计 → 开发
- [ ] DB 表 `tb_openspec` + 字典 + 菜单
- [ ] 前端 `plm-frontend/packages/plm-openspec/` 实现 api / types / views
