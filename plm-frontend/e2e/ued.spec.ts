...
test.describe('UED 模块 E2E (PRD §F2.3)', () => {
  ...
  test('TC-UED-F001 创建 UED 设计稿 (Web 大屏 + UI 设计类型)', async () => {
    const r = await api.post('/business/ued', {
      projectId,
      title: `农情大屏设计稿-${RUN_ID}`,
      designType: 'ui',
      platform: 'web',
      version: 'v1.0',
      description: '农情大屏 PC 端 UI 设计,包含土壤墒情、气象数据、灌溉推荐三个核心模块的可视化设计。',
      designerUserId: 1
    })
    expect(r.code).toBe(200)
  })

  test('TC-UED-F002 创建 UED 设计稿 (移动端 + UE 交互设计)', async () => {
    const r = await api.post('/business/ued', {
      projectId,
      title: `移动端农事记录UE稿-${RUN_ID}`,
      designType: 'ue',
      platform: 'mobile',
      figmaFileKey: 'test-figma-key-12345',
      figmaUrl: 'https://www.figma.com/file/test-key/mobile-farming',
      version: 'v1.0',
      designerUserId: 1
    })
    expect(r.code).toBe(200)
